...
`yuexianwei3699` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/yuexianwei3699/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/yuexianwei3699/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee yuexianwei3699` to
...
python ./.trellis/scripts/task.py archive 00-join-yuexianwei3699
```
