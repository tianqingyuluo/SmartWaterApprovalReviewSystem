...
`6newuser6` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/6newuser6/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/6newuser6/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee 6newuser6` to
...
python ./.trellis/scripts/task.py archive 00-join-6newuser6
```
