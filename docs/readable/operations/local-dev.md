...
前端登录后，申请人列表页会显示办理提醒栏：

- `CORRECTION_REQUIRED` 的任务会出现在提醒栏中，并可跳转到结果页补传材料。
- `INITIAL_REVIEW_PASSED` 的任务会显示通过初审提示，便于申请人确认申请单已进入后续流程。
- `MANUAL_REVIEW_REQUIRED` 的任务会显示人工复核提示，帮助申请人理解当前状态。

如果当前账号是审批人员或管理员，列表页不会展示申请人专属的办理提醒栏。
